...
- Fable is advisory, not a Baton participant: [skill-only interface design](../superpowers/specs/2026-08-27-skill-only-run-interface-design.md). Its older rule limits Fable to Claude-hosted prativadi and genuine unresolved disagreement; the current both-role, before-every-real-Decision behavior is a human-approved extension, not source equivalence.
