...
>
> **Spec rev 2026-07-12:** Full-profile re-laps use Option-B delta re-verification at exactly one intermediate gate. The first pass is full; only mechanical `test_creation` tracks may carry when their same-id origin, current-cycle ancestry, declared `covers_chunks`/`carry_reason`, and engine-derived `git-covers-diff-v1` tracked regular-file closure validate unchanged. Reviews, risk angles, global/unbounded tracks, and all `verification_matrix` rows never carry. The terminal `done` gate remains a full re-verification boundary, and absent carry fields retain legacy behavior.
>
> **Spec rev 2026-07-01:** Wait-helper human intervention is now a pause-active-work-together invariant. `human_question` and `human_decision` are a paired pause of active work on both roles, not a stop of the polling loop — a Codex-hosted role keeps its `--through-human` wait running through the pause and resumes automatically, and exiting the wait loop at a human pause is a protocol violation. During any selected non-terminal wait, a newer sibling run's `human_question` or `human_decision` is propagated to the selected waiter unless `DVANDVA_CONCURRENT=1`; older sibling human-intervention batons stay parked, and sibling `human_question` output preserves `question`, `resume_assignee`, and `resume_status`. Post-handoff waits use `--since-checkpoint <written_checkpoint> --until-actionable` so a team-owned `active_roles` state cannot bounce the writer back to ready on the same checkpoint or wake a role before it has dependency-unblocked work.
>
...
> plugin ships the hook; the `dvandva` binary must be on `PATH`). Engine patches (v2 development):
...
> a Codex-hosted role goes silent but keeps its `--through-human` wait running
> through the pause and resumes automatically when the answer lands (exiting the
> wait loop at a human pause is a protocol violation), and if no
...
> sibling_run_id=<id>]` line once (deduped across a persist-max/shell-budget
> re-invocation via a `.wait-pause-<role>` marker file beside the baton),
> suspends the stall watchdog for the duration, and resumes normal
> semantics — including `--until-actionable` —
...
> checkpoint=... age_s=... root=...` line on every scan that finds it — a
> stateless scanner with no dedup or pacing; cron logs are the record. A
> `DVANDVA_WATCHDOG summary roots=... batons=... stale=... paused=...
> skipped=...` line always prints last, and the process always exits 0 — it
> is a monitor, not a gate.
>
> **Spec rev 2026-07-03 (notify retirement):** webhook notification channel
...
Full-profile implementation-phase parallelism is mandatory in v2. Spec approval enters `parallel_implementing` with `assignee: "team"` and `active_roles: ["vadi", "prativadi"]`; the phase `work_split` must contain at least five implementation chunks split across both roles for two-team parallel implementation. Every implementation chunk names reciprocal `cross_review_by`, and `test_creation` routes to `cross_review`. If cross-review finds peer-chunk defects, the phase routes through `cross_fixing` and then back to `test_creation` before review continues. If both reciprocal tracks approve a clean non-final phase, the baton advances exactly to phase N+1 at the target phase's profile-appropriate implementation state without an Opus dispatch; material risk may instead escalate to `deep_review`, and the final phase must do so. Fast and standard profiles use the compact `implementing -> phase_review -> termination_review -> done` path, still with `profile_decision`, passing final verification, completed `verification_matrix` evidence, completed approved prativadi `phase-review` evidence with current-cycle `review_checkpoint`, shared termination, and role-owned final approvals.
...
Legacy enforcement starts with the agent checklist embedded in each SKILL.md and `/goal` evaluator transcript checks. The bundled write helper enforces `dvandva.baton.v3` as the sole write contract — required fields, checkpoint arithmetic, safe run IDs, status-owner pairs, and transition subset; `dvandva.baton.v1` and `dvandva.baton.v2` candidates are rejected with `schema_retired` and a migration hint, and survive only on the lenient READ path. A future standalone CLI validator backed by a full JSON Schema file can replace the remaining checklist-only validation.
...
The dated advisory ranking table for concrete model choice lives in `docs/model-selection.md`; it informs casting defaults and escalation, not validator semantics or seed-agent retiering.
...
- `dvandva write` (`rust/dvandva/src/write.rs`) — validated atomic baton installer; validates `dvandva.baton.v3` transitions and rejects retired v1/v2 candidates with `schema_retired`, installs via tmp+rename, auto-snapshots. Covered by the write port's `cargo test` suite.
...
The full-profile v2 flow has eight segments available after a mandatory clarifying-questions prefix and an end state: research, master planning, implementation, test_creation, cross_review, risk/final deep_review, deslop, and phase advancement/completion. Clean non-final cross-review takes the direct phase-advance branch; risk escalations and the final phase take the deep-review branch. Fast and standard profiles use the compact `implementing -> phase_review -> termination_review -> done` path after the mandatory clarifying prefix and research/spec prelude. Every arrow in the diagram is a baton write by the active agent. In default walkaway mode, the other persistent session is already blocked in `dvandva wait`; the helper returns when the baton assigns that role, and the agent re-enters preflight.
...
```
  ...
  │   Team (parallel_implementing phase N)                │
   ...
   │     defects → cross_fixing                             │
   │     clean non-final → phase N+1 implementation        │
   │     risk or final → deep_review                        │
   │       ▼                                               │
   │   Prativadi (risk/final deep_review for phase N)      │
   │     credited Opus review after tests exist            │
   │     final phase reads cumulative branch-baseline diff │
   ...
   │                                                     │ │
   └─── on phase N+1 ──▶ profile-appropriate implementation│ │
                                                         │ │
                                                         ▼ ▼
                                                  ┌───────────┐
                                                  │  human    │
...
Phase advancement invariant: the vadi never advances a phase directly after implementation or fixing. A full-profile clean non-final `cross_review` advances exactly to phase N+1 at the target phase's profile-appropriate implementation state. A risk-escalated or final `cross_review` enters `deep_review`; its approval routes through `deslop`, where a clean non-final checkpoint starts the next phase and a clean final checkpoint enters shared `termination_review`. Legacy v1 direct advancement remains valid only for explicitly legacy runs. The agent writing the first baton for the next phase must set `disagreement_round: 0`.
...
2. **Preflight** — read `AGENTS.md`, resolve the baton path from `DVANDVA_BATON_FILE`, `DVANDVA_RUN_DIR`, safe `DVANDVA_RUN_ID`, then Existing baton discovery across `.dvandva/runs/*/baton.json` plus legacy `.dvandva/baton.json`, and set `BATON_FILE` plus `BATON_NEXT_FILE`. If no explicit selector exists and exactly one active/resumable baton exists, select it; if several exist, surface the candidates and wait for a human choice; if none exists, wait continuously on the selected or would-be named-run baton with `--allow-missing` unless `DVANDVA_NO_WAIT=1` is set. When no baton path is knowable at all (only terminal archives, or an empty runs directory), do not stop and do not scaffold: enter discovery wait with `dvandva wait --role prativadi --discover --interval 60 --max-wait 540 --stall-max 1800 --until-actionable` (Codex-hosted sessions append `--through-human`); the helper adopts the sole new non-terminal baton and continues normal wait semantics, or exits 14 (`discover_ambiguous`) on several candidates, and `dvandva preflight --role prativadi` answers `result=wait` with this command when there is nothing to resume. If the baton is assigned away and `run_mode: "walkaway"`, wait continuously on `"$BATON_FILE"` until role ownership, terminal baton state, or user interrupt; shell caps must re-enter the wait. If `run_mode: "supervised"`, exit on assigned-away states so the human can invoke the next role. **Additionally verify `superpowers:brainstorming` is available in the current session** before spec Q&A; if absent, surface install instructions and exit (per section 4 prerequisites). Do not depend on one fixed filesystem path.
...
Agent files are separate from `SKILL.md` portability. Their `model:` field is a Dvandva model-class hint and workload-routing label, not an engine lock-in or ranked model table. Claude Code maps `opus` to Opus-class, `sonnet` to Sonnet-class, `fable` to Fable-class, and `gpt` to a Sonnet-class wrapper that shells to Codex where available. Codex maps `opus` and `fable` to `gpt-5.6-sol` and `sonnet` and `gpt` to `gpt-5.6-terra` on the active surface; Dvandva does not automatically fall back to older model generations, and an unavailable required model routes to `human_decision`. Codex-side `opus` and `fable` executions are GPT hygiene only and never earn review credit; credited deep/adversarial review remains a cross-vendor Anthropic Opus gate. Codex reasoning effort is keyed to the thread role rather than the model class: the main session defaults to `xhigh` on every model and requests `max` only when the human sets it explicitly, while every dispatched Codex child is launched with an explicit `xhigh` effort because omitting it inherits the parent, may be lowered to `high`, `medium`, or `low` for proven-mechanical work, and never requests `max`; no Dvandva role uses `ultra` because its Codex-managed delegate threads run outside the baton's two-role coordination, and when a model does not support the requested effort the dispatching role keeps the selected model, drops to that model's highest supported effort, and logs the requested effort, effective effort, and reason. All current Codex models support `xhigh`; `max` is supported by `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`; `ultra` is supported only by `gpt-5.6-sol` and `gpt-5.6-terra` but is forbidden in Dvandva because it visibly performs proactive delegation outside the baton's two-role coordination. Use `opus` for architecture, planning, deep review, adversarial/security/integration/doc-verification, and baton-audit work. Use `sonnet` for bounded implementation, documentation, verification, routine cross-review, debugging, test creation, sandbox probes, and deslop. Dvandva never maps any agent to a `haiku` class.
...
|---|---|
| No selected/resumable baton after discovery | Vadi creates a named `dvandva.baton.v3` run at `phase: "clarifying", status: "clarifying_questions_drafting"`; the legacy `.dvandva/baton.json` is now read-only (its v1 WRITE path is retired), so no seed is scaffolded there. Prativadi waits on the selected or would-be named-run baton with `--allow-missing` unless `DVANDVA_NO_WAIT=1`; it does not create the run. When no run path is knowable, prativadi uses `dvandva wait --role prativadi --discover` (adopt-and-continue; exit 14 `discover_ambiguous` on several new candidates) instead of stopping. |
...
| `schema` field is outside `{dvandva.baton.v1, dvandva.baton.v2, dvandva.baton.v3}` | Both: refuse to operate. Surface schema mismatch. Exit. On the WRITE path, `dvandva.baton.v1`/`v2` are additionally rejected with `schema_retired` — only `dvandva.baton.v3` is writable. |
...
| `status: human_question` or `human_decision` written or observed (F5) | The Claude Code-hosted session owns surfacing human_question and human_decision to the human. Whichever role the Claude Code session hosts asks the human directly in-session (question, options, resume fields) — on writing the pause or on wait exit 11/12 (including sibling propagation) — and stays available for the answer via Claude Code's mobile/remote surface. A Codex-hosted role goes silent but keeps its `--through-human` wait running through the pause and resumes automatically when the answer lands — exiting the wait loop at a human pause is a protocol violation; it must not compete to consume the answer. If no Claude Code session is part of the run, the writer of the pause surfaces it. `current_engine` still records the writer for traceability. |
...
| Write helper exits 23 `schema_retired` | The candidate (or the current baton) carries `schema: "dvandva.baton.v1"` or `"dvandva.baton.v2"`; both WRITE paths are retired (v1 per S5-T2/D5, v2 with the v3 per-run-workflow schema), so `dvandva.baton.v3` is the sole writable schema. Migrate the candidate to `dvandva.baton.v3` and rerun (the helper hint chains v1→v2→v3). Old v1/v2 batons stay readable on the `state`/`resolve`/`wait` path. |
...
- **Schema key-presence check** (same script): the inlined `dvandva.baton.v3` JSON in each SKILL.md must parse as valid JSON and contain the required keys from Appendix A. Not a JSON Schema check — that's v2.
...
- **Schema version:** `dvandva.baton.v3` is the sole WRITE/scaffold contract for named runs; `dvandva.baton.v1` (legacy `.dvandva/baton.json`) and `dvandva.baton.v2` are retired from the WRITE path (rejected with `schema_retired`) and remain valid only on the READ path for observing old batons. Breaking changes increment the schema version; both skills must update in lockstep. Skills refuse to operate on a schema string outside the supported set (section 12).
- **Schema maintenance.** The `dvandva.baton.v3` write contract, plus the historical v1/v2 read-path copies, are spread across many hand-maintained files, all held in parity by `dvandva lint schema-parity` (S6-T1): the engine's own `dvandva.baton.v3` status enum and `v2_required_keys()` + `run_workflow` (`rust/dvandva/src/write.rs`); `plugins/dvandva/references/baton-schema-v3.json` (its `status_catalog`, pinned to the live 29-token v3 catalog) and the historical `plugins/dvandva/references/baton-schema-v2.json` (its 26-token read-path `status_catalog`); the `Status catalog (26)` marker line in this spec (Appendix A) and in `plugins/dvandva/references/state-transition-table.md`; the inline `json` seed block in each of `plugins/dvandva/skills/vadi/SKILL.md` and `plugins/dvandva/skills/prativadi/SKILL.md` (top-level keys ≡ `v2_required_keys()` plus `run_workflow`, one `json` fence per file); the byte-identical channel-doc copies `docs/protocol/local-baton-channel.md` and `plugins/dvandva/references/local-baton-channel.md`; and the HISTORICAL references `plugins/dvandva/references/baton-schema.json` and `templates/channel/baton.json` (each carrying a `HISTORICAL: dvandva.baton.v1` marker) plus `baton-schema-v2.json` (its `HISTORICAL: dvandva.baton.v2` marker). On a schema change, update the engine first, then every copy above, and run `dvandva lint schema-parity` and `dvandva lint skills` until green.
...
| `cross_review` | `phase: N+1, status: "parallel_implementing"` | Both current-cycle reciprocal tracks approve, N is non-final, and the target phase's effective profile is full; no Opus dispatch request is created. |
| `cross_review` | `phase: N+1, status: "implementing"` | Both current-cycle reciprocal tracks approve, N is non-final, and the target phase's effective profile is standard; no Opus dispatch request is created. |
| `cross_review` | `deep_review, review_target: implementation` | Both roles recorded completed approved cross-review tracks with evidence, and the phase is final or a non-final risk escalation is requested. The existing credited-Opus dispatch lifecycle remains mandatory. |
...
**Legacy v1 explicit fallback (read-path history):** The v1 WRITE path is retired with `schema_retired`, so these seeds can no longer be written; the table documents the transitions of pre-existing v1 batons only.
