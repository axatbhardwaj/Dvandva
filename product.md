...
8. **Phase advancement or completion** — on agreement, make a regular local checkpoint commit for the verified logical slice when `allow_commit` permits it, then advance to phase N+1. On completion of the final phase, write `./superpowers/run-reports/YYYY-MM-DD-<run_id>-explainer.html`, set `run_explainer_ref`, require both roles to review that exact explainer through `run_explainer_reviews`, require both final approvals, optionally push, then transition to `done`.
