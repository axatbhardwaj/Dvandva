...
10. **Stop rule** — in walkaway mode, do not stop on role handoff. Surface BATON_STATE_COMPACT via `dvandva-state.sh --compact`, run the wait helper, and continue from preflight when the baton returns. In supervised mode, exit on role handoff.
...
2. **`/goal` evaluator misjudges baton state surface.** If Claude or Codex surfaces a baton-state line in a way the evaluator misreads, the loop may stop prematurely or fail to stop. Mitigation: skill bodies require the bounded `BATON_STATE_COMPACT` JSON summary from `dvandva-state.sh --compact` at every checkpoint, instead of full dynamic baton arrays.
