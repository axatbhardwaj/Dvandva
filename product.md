...
- **Run 4 — generalized path-gate + retire standalones + work-gating.** Off-protocol commits are guarded by role-preflight-installed local git hooks plus a `dvandva.hooksAdoptedAt` drift-lint baseline, generalized `work_split` path-gate enforcement extends beyond the Run 3 dynamic disjointness check, and the user's standalone agent fleet is retired only for Dvandva-covered workflows with functional parity via Runs 1-4 usage, backup-manifest reversibility, Codex agent-axis no-op behavior, and no skill touches.
