...
Agent files are separate from `SKILL.md` portability. Their `model:` field is a Dvandva model-class hint, not an engine lock-in: `opus` means the strongest planning, architecture, and deep/hard-risk review class, and `sonnet` means the implementation, documentation, research, verification, and routine-cross-review workhorse class. Claude Code maps `opus` to Opus-class and `sonnet` to Sonnet-class models. Codex maps `opus` to `gpt-5.5` and `sonnet` to `gpt-5.4`, and runs both classes at `xhigh` reasoning effort. Dvandva never maps any agent to a `haiku` class.
