...
2. Resolve the active baton path before reading or writing:
   ...
   - Set `BATON_DIR="$(dirname "$BATON_FILE")"`, `BATON_NEXT_FILE="$BATON_DIR/baton.next.json"`, and `BATON_BROKEN_FILE="$BATON_DIR/baton.broken.json"`. Preserve `run_id`, `original_ask`, `research_ref`, `work_split`, and `verification_matrix` when they already exist on the baton; surface them every turn.
3. Read `$BATON_FILE`. If the file does not exist, scaffold it immediately:
   ...
   - For the legacy `.dvandva/baton.json` fallback, write the canonical `dvandva.baton.v1` seed at the bottom of this skill with `phase: "spec"`, `status: "spec_drafting"`, `assignee: "vadi"`, `checkpoint: 0`, the same `updated_at`/`run_mode` handling, and the user's original ask in `summary` so prativadi can prepare independently.
...
|---|---|
| `research_drafting` / `research_revision` | `dvandva-researcher`, `dvandva-architect`, `dvandva-baton-auditor` |
| `spec_drafting` / `spec_revision` | `dvandva-architect`, `dvandva-baton-auditor` |
| `implementing` | `dvandva-implementer`, `dvandva-sandbox-verifier` when runtime evidence helps |
...
3. Use parallel subagents for codebase, docs/protocol, verification, risk, and work-distribution tracks when available. If no subagent tool is available, do the same exploration directly and record the fallback in work_split.
...
3. Implement only the scope listed for that phase. Do not bleed into adjacent phases.
...
- `status: "phase_review"` for the current v1 helper. In v2, use `status: "test_creation"` first, then `status: "deep_review"` after tests are created.
- `assignee: "prativadi"`
- `current_engine`: set to `"claude"` if you are Claude Code, or `"codex"` if you are Codex. This is for traceability only.
...
- `next_action: "Prativadi: perform deep_review for phase <N> after implementation and test_creation. Apply narrow fixups within the allowlist, route deslop findings, or hand back substantive issues."`
...
4. Write the next baton as `status: "deep_review"`, `assignee: "prativadi"`, and `review_target: "implementation"`; `dvandva-write.sh` validates this live v2 transition.
...
- `phase: <current N>` (unchanged)
- `status: "phase_review"` for current v1 helper compatibility; v2 should route through `test_creation` before `deep_review`.
- `assignee: "prativadi"`
- `current_engine`: set to `"claude"` if you are Claude Code, or `"codex"` if you are Codex. This is for traceability only.
- `review_target: "implementation"`
...
3. Re-run affected tests or lints and update `verification_matrix`.
4. If cleanup uncovers substantive behavior or architecture risk, route to `phase_fixing` instead of advancing.
5. If no issues remain except explicitly accepted `deferred` entries, advance to the next phase or final completion.
...
2. Immediately run a foreground wait against the resolved `"$BATON_FILE"`. Claude Code uses finite waits: `${CLAUDE_SKILL_DIR}/scripts/dvandva-wait.sh --role vadi --file "$BATON_FILE" --interval 60 --max-wait 540` with an explicit 600000 ms Bash-tool timeout; on exit 20, surface the heartbeat and run it again unless interrupted. Codex-hosted sessions may use `--persist`: `${CLAUDE_SKILL_DIR}/scripts/dvandva-wait.sh --role vadi --file "$BATON_FILE" --interval 60 --max-wait 540 --persist`; add `--persist-max <600` when running inside a Claude-hosted shell. The wait-helper persist cap exit 23 is a persistent wait cap, not a terminal baton state.
