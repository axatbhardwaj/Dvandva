...
**Binary presence (before anything else):** verify the `dvandva` binary is on `PATH` with `command -v dvandva`. If it is not found, surface the install instruction: install it with `cargo install --path rust/dvandva` from a Dvandva checkout, or `cargo install dvandva --version 2.0.0-alpha.2` once published — the multicall `dvandva` binary is the single Dvandva runtime — and STOP without resolving, scaffolding, or writing a success or advancement baton (mirror the Superpowers-absent failure mode).
...
New scaffolds write `"development"` for `mode`; `"feature-pr"` is the legacy alias accepted by `dvandva write` for existing batons.
```json
{
