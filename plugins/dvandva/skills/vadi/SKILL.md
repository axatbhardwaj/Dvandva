...
5. Run `dvandva preflight --role vadi` before active non-wait work. Set `export DVANDVA_ROLE=vadi` first; the preflight asserts `DVANDVA_ROLE=vadi` (exits 1 on mismatch). This is the single turn-entry gate: it resolves the active run selector-first (stopping on exit 12 ASK) then runs the hook stage. On exit 12 (ASK), surface the candidate runs and stop this turn; on exit 1 (blocking hook), follow the stated reason to `human_decision`. The hook stage detects Dvandva hook adoption status; it records the prior `core.hooksPath` as `dvandva.priorHooksPath`, sets `core.hooksPath` to `.dvandva/githooks` (a delegating wrapper), execs the prior hook chain on every commit so the foreign owner keeps firing, and restores the prior `core.hooksPath` on uninstall — preserving the existing hooks configuration through record, delegate, and restore. Checkpoint commits require Dvandva hook adoption (the delegating wrapper active). Final commits require Dvandva hook adoption.
   ...
   Baton creation has a vadi-owned SLA: `dvandva resolve --role vadi` or `dvandva preflight --role vadi` returning `CREATE` (including explicit selector bootstrap for a missing `baton.json`) arms `.dvandva/.session-baton-pending.vadi`, with threshold `DVANDVA_BATON_SLA_SECONDS` defaulting to 120 seconds and the arm path printing `DVANDVA_SLA armed ...` with the deadline. After the deadline, the Claude-hosted `baton-guard` emits repeated model-visible warnings and never blocks ordinary tools; Codex has no hook surface, so it sees the countdown only from the printed deadline line. Create or resume the baton promptly by writing the seed candidate or keep waiting only when deliberately peer-seeded. If a selector-targeted run directory contains stale bootstrap state (`invalid_baton`, `invalid_candidate`, or `garbage_marker`), preflight stops with `reason=stale_run_dir` so the human can inspect before anything is overwritten.
...
|---|---|
| `research_drafting` / `research_revision` | `gpt-5.6-sol` using `dvandva-researcher`, `dvandva-pattern-mapper`, `dvandva-architect`, and `dvandva-baton-auditor` as role briefs; the concrete Sol assignment overrides their seed model classes |
...
3. Dispatch `gpt-5.6-sol` for the codebase, docs/protocol, verification, risk, and work-distribution research tracks; parallelize only genuinely disjoint tracks. Sol produces the research content; the vadi coordinates the tracks and does not replace them with Fable- or Sonnet-authored research. An optional Grok lane may supply read-only freshness leads for Sol to verify.
4. Serialize Sol's result into `research_ref` at `./superpowers/research/YYYY-MM-DD-<topic>.html` as a dark self-contained HTML artifact with machine-readable metadata. Only the vadi writes the artifact and baton.
...
3. Re-dispatch `gpt-5.6-sol` for targeted research revisions. An optional Grok lane may supply read-only freshness leads for Sol to verify.
4. Serialize Sol's revised content into `research_ref`, `work_split`, `verification_matrix`, and any affected development profile fields; keep test creation separate from review.
...
4. If both vadi and prativadi cross-review tracks are current-cycle, completed, and approved, choose the cadence branch. Clean non-final: advance exactly to phase N+1 without adding a `dispatch_requests` entry; use `parallel_implementing` / team / both active roles when the next effective profile is full, or `implementing` / vadi / empty active roles when it is standard, preserving the target owner shape and resetting `disagreement_round: 0`. Material correctness, security, integration, protocol, or explicitly requested non-final risk: keep the current phase and write `deep_review` / prativadi / empty active roles / `review_target: "implementation"`. Final phase: the direct implementation edge is forbidden; enter that same mandatory `deep_review` state. A `deep_review` entry records an open vadi `dispatch_requests` entry for credited cross-vendor Anthropic-Opus dispatch; direct advance does not. The waiter wakes (`DVANDVA_WAIT dispatch_requested`), then vadi runs `dvandva next --to deep_review --summary "<ack note>" --next-action "<next>"` to acknowledge before dispatch; only the matching `DVANDVA_ROLE` may ack, deep-review exit completes the request, and `--dispatch-request <id>` resolves a non-canonical tie. Final Opus intake compares the branch baseline to the working tree with `git diff <baseline>` and inventories `git status --short`; every intended untracked file must be opened and included in the review evidence.
