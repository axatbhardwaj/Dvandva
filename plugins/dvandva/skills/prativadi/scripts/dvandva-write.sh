...
# validates the candidate (schema, required keys, status enum, v1 transition
...
if ! jq -e '.schema == "dvandva.baton.v1"' "$CANDIDATE_FILE" >/dev/null 2>&1; then
  echo "DVANDVA_WRITE schema_mismatch candidate=$CANDIDATE_FILE want=dvandva.baton.v1" >&2
  exit 23
fi
...
case "$new_status" in
  spec_drafting|spec_review|spec_revision|human_question|implementing|phase_review|phase_fixing|review_of_review|counter_review|human_decision|done) ;;
  *)
...
if [[ ! -f "$BATON_FILE" ]]; then
  ...
  if [[ "$new_status" == "spec_drafting" && "$new_assignee" == "vadi" && "$new_checkpoint" -eq 0 ]]; then
    legal=1
  else
    reason="scaffold requires status=spec_drafting assignee=vadi checkpoint=0, got status=$new_status assignee=$new_assignee checkpoint=$new_checkpoint"
  fi
else
  ...
  if ! cur="$(jq -r '[.status // "", (.checkpoint // -1 | tostring), (.master_plan_locked // false | tostring), .resume_assignee // "", .resume_status // ""] | @tsv' "$BATON_FILE" 2>/dev/null)"; then
    echo "DVANDVA_WRITE current_baton_unparseable file=$BATON_FILE refusing_to_overwrite=true" >&2
    exit 25
  fi
  IFS=$'\t' read -r cur_status cur_checkpoint cur_locked cur_resume_assignee cur_resume_status <<< "$cur"
  ...
  if [[ "$new_checkpoint" -ne $((cur_checkpoint + 1)) ]]; then
    ...
    elif [[ "$cur_status" != "spec_drafting" && "$cur_status" != "spec_review" && "$cur_status" != "spec_revision" ]]; then
      reason="human_question only enters from spec states, not $cur_status"
    ...
    case "${cur_status}:${new_status}" in
      ...
      *) reason="no v1 edge ${cur_status}->${new_status}" ;;
    esac
  fi
fi
