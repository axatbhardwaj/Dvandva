...
**Binary presence (before anything else):** verify the `dvandva` binary is on `PATH` with `command -v dvandva`. If it is not found, surface the install instruction: install it with `cargo install --path rust/dvandva` from a Dvandva checkout (primary; builds `2.0.0-alpha.6`), or the latest published release with `cargo install dvandva --version 2.0.0-alpha.5` — the multicall `dvandva` binary is the single Dvandva runtime — and STOP without resolving, scaffolding, or writing a success or advancement baton (mirror the Superpowers-absent failure mode).
