...
5. Enforce repo-local Dvandva git hooks before active work in a git repo: from the repository root, run `bash scripts/install-dvandva-hooks.sh`, then assert the repo-local config is `core.hooksPath=.githooks` (`git config --local core.hooksPath` returns `.githooks`). The installer records `dvandva.hooksAdoptedAt` as the local drift-lint baseline. If install or verification fails while prativadi owns the baton, write `status: "human_decision"` with the failing command in `blockers`; otherwise surface the setup blocker and exit.
