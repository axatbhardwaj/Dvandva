...
| `dvandva-wait.sh` exits 24 (`stalled`) | `--stall-max` seconds elapsed without the baton advancing — a stalled or dead peer. Write `status: "human_decision"` naming the stall, then stop. This is a `dvandva-wait.sh` code; it is **distinct from** `dvandva-write.sh` exit `24` (illegal transition). |
