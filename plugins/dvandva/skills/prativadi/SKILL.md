...
4. If both vadi and prativadi cross-review tracks are completed and approved, hand the baton to `deep_review` with `assignee: "prativadi"` and `active_roles: []`. That deep_review-entering write records an open `dispatch_requests` entry (`role: "vadi"`, purpose: credited cross-vendor Anthropic-Opus dispatch); the vadi's waiter wakes on it (`DVANDVA_WAIT dispatch_requested`) and the entry is completed when the credited reviews land. The addressed vadi — not the prativadi holding deep_review — runs `dvandva next --to deep_review --summary "<ack note>" --next-action "<next>"` (`--to` requires both flags, exit 2 otherwise) after waking to emit the same-status ack write flipping its request `open -> acknowledged` before dispatching the reviewers; the ack leaves deep_review prativadi-owned and only the entry's own role may write it, and either role closes the request (`completed`) on the deep_review exit. The bare command acks the canonical credited-Opus request for the effective role (`--role`, else DVANDVA_ROLE); when several open requests for that role tie with none canonical, pass `--dispatch-request <id>` to select one.
