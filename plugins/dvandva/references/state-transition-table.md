...
| `cross_review` | `phase: N+1, status: "parallel_implementing"` | Both current-cycle reciprocal tracks are completed and approved, N is non-final, the next effective profile is full, and the target is team-owned. This direct clean-phase advance creates no credited-Opus dispatch request. |
| `cross_review` | `phase: N+1, status: "implementing"` | Both current-cycle reciprocal tracks are completed and approved, N is non-final, the next effective profile is standard, and the target is vadi-owned. This direct clean-phase advance creates no credited-Opus dispatch request. |
| `cross_review` | `deep_review` | Both roles recorded completed cross-review tracks for peer-owned chunks, and either a non-final risk escalation is requested or the phase is final. Dispatch-request lifecycle: this entry write PRODUCES an open `dispatch_requests` entry (`role: "vadi"`, status `open`) — `dvandva next --to deep_review` scaffolds it and the write gate rejects the entry without it (`missing_dispatch_request`, exit 23); the vadi's waiter CONSUMES it (`DVANDVA_WAIT dispatch_requested`); after waking the addressed vadi ACKNOWLEDGES it (`status: open -> acknowledged`) via `dvandva next --to deep_review` — the same-status ack write that claims the paid dispatch so the wait stops re-firing — before dispatching the credited cross-vendor Opus reviewers; and every non-escape exit from `deep_review` must CLOSE it (`status: completed|cancelled`) or the write is rejected (`open_dispatch_request`, exit 23). So the full per-id lifecycle is produced -> woken -> acknowledged -> closed. Identity (`role`+`purpose`) is immutable for the life of an id (mutation is rejected `dispatch_identity_mutated`); the entry gate binds to the EXACT canonical purpose (a substring is not the signal); only DVANDVA_ROLE equal to the entry's role may ack; `acknowledged` is still-open for the closure gate; and per-id transitions follow open->acknowledged/completed/cancelled, acknowledged->completed/cancelled, with completed/cancelled terminal (`bad_dispatch_transition`). Escape exits (`human_question`/`human_decision`/`abandoned`) ride the open request through. |
...
- **Delta re-verification (Option B).** The first full-profile pass executes every gate. On a re-lap through `phase_fixing` or `cross_fixing`, only a mechanical `test_creation` track may satisfy the intermediate test gate by carry. Carry requires `covers_chunks`, `carry_reason`, same-id origin provenance at `carried_from_checkpoint`, current-cycle ancestry, and an unchanged engine-derived closure proven by `git-covers-diff-v1` over tracked regular non-symlink files; any failure reruns under a new track id. Cross-review, deep-review, risk-angle, global/unbounded tracks, and every `verification_matrix` row never carry. The terminal `done` gate is unchanged and still requires full review depth plus every matrix row fresh after the latest implementation-family checkpoint. Batons without carry fields retain legacy full-rerun behavior.
...
Model classes are durable workload-routing classes, not a literal ranked model table. Canonical compatibility strings remain vendor-neutral, and legacy aliases must keep validating existing batons.

| Routing class | Accepted class string | Claude Code mapping | Codex mapping |
|---|---|---|---|
| `opus` | `opus-class\|gpt-5.5-xhigh` | Opus-class | gpt-5.6-sol |
