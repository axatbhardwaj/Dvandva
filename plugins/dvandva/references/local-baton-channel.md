...
- A joining role that finds no active or resumable baton (only terminal archives, or an empty runs directory) does not stop and does not scaffold: it enters discovery wait with `dvandva wait --role <vadi|prativadi> --discover --interval 60 --max-wait 540 --stall-max 1800 --until-actionable` (Codex-hosted sessions append `--through-human`). The helper polls `.dvandva/runs/*/baton.json` and the legacy path until a non-terminal baton appears, adopts it, and continues normal wait semantics; if several new active batons appear it exits 14 (`discover_ambiguous`) and surfaces the candidates for explicit selection. `dvandva preflight --role prativadi` answers `result=wait` with this command when there is nothing to resume; `--discover` is exclusive with `--file`.
