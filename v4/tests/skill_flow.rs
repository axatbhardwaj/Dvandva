...
fn workflow_modes_define_distinct_review_and_authority_contracts() {
    ...
    for (role, source, normalized) in [
        ...
        assert!(
            normalized.contains("optionally consult local Astra or Fable"),
            "{role} must keep planning advice optional"
        );
        assert!(
            !source.contains("Fable adjudication"),
            "{role} must not restore the mandatory Fable escalation gate"
        );
    }
    for required in [
...
fn prativadi_review_inputs_and_verdict_are_immutably_bound() {
    ...
    let workflow = repository_file("docs/workflows/skill-only-run.md")
        ...
        .collect::<Vec<_>>()
        .join(" ");
    assert!(workflow.contains("Dvandva setup installs only the four Dvandva skills"));
...
fn version_and_probe_report_the_installation_contract() {
    command()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("dvandva-v4 0.4.2"));
