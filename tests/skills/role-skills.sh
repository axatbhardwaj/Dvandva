...
binary="$XDG_DATA_HOME/dvandva/bin/0.4.2/dvandva-kernel"
mkdir -p "$(dirname "$binary")"
cp "$repo_root/v4/target/debug/dvandva-v4" "$binary"
ln -s 0.4.2 "$XDG_DATA_HOME/dvandva/bin/current"
...
if test "${1:-}" = "--version"; then
  case "${DVANDVA_FAKE_MODE:-valid}" in
    valid|probe_*) printf 'dvandva-v4 0.4.2\n' ;;
    version_nul) printf 'dvandva-v4 0.4.2\0\n' ;;
    version_invalid_utf8) printf 'dvandva-v4 0.4.2\377\n' ;;
    version_oversized) printf 'dvandva-v4 0.4.2'; head -c 300 /dev/zero | tr '\0' x ;;
    version_extra_newline) printf 'dvandva-v4 0.4.2\n\n' ;;
    version_nonzero) printf 'dvandva-v4 0.4.2\n'; exit 7 ;;
  esac
  exit 0
fi
