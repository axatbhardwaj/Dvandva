...
CARGO_TARGET_DIR="$test_root/target" bash "$packager" skills-v0.1.0 "$output"
...
test "$($output/dvandva-kernel-linux-x86_64 --version)" = 'dvandva-v4 0.1.0'
...
if CARGO_TARGET_DIR="$test_root/target" bash "$packager" skills-v0.1.0 "$nonempty" \
