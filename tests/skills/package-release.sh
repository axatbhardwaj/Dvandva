...
printf '%s\n' \
  ...
  '    test "$2" = "--verify"' \
  '    test "$3" = "--hash"' \
  '    test "$4" = "refs/tags/skills-v0.4.2"' \
  ...
  '    test "$#" -eq 3' \
  '    test "$2" = "--verify"' \
  '    test "$3" = "refs/tags/skills-v0.4.2^{commit}"' \
  ...
  '    test "$3" = "origin"' \
  '    test "$4" = "refs/tags/skills-v0.4.2"' \
  '    test "$5" = "refs/tags/skills-v0.4.2^{}"' \
...
expect_ref_accepted() {
  ...
  if ! PATH="$ref_fake_bin:$PATH" \
    ...
    bash "$ref_verifier" skills-v0.4.2 origin \
...
expect_ref_accepted lightweight-tag \
  "$release_commit\n" "$release_commit\n" \
  "$release_commit\trefs/tags/skills-v0.4.2\n"
expect_ref_accepted annotated-tag \
  "$release_tag_object\n" "$release_commit\n" \
  "$release_tag_object\trefs/tags/skills-v0.4.2\n$release_commit\trefs/tags/skills-v0.4.2^{}\n"
...
expect_ref_rejected moved-remote-object \
  "$release_tag_object\n" "$release_commit\n" "$release_commit" \
  "$moved_object\trefs/tags/skills-v0.4.2\n$release_commit\trefs/tags/skills-v0.4.2^{}\n"
...
expect_ref_rejected malformed-remote \
  "$release_commit\n" "$release_commit\n" "$release_commit" \
  "not-an-object\trefs/tags/skills-v0.4.2\n"
...
git -C "$noncommit_repo" update-ref refs/tags/skills-v0.4.2 "$tree_object"
...
git -C "$noncommit_repo" push -q origin \
  refs/tags/skills-v0.4.2 refs/tags/skills-v0.2.1
for noncommit_tag in skills-v0.4.2 skills-v0.2.1; do
...
printf '%s\n' \
  ...
  "  printf '%s\\n' '{\"package\":\"dvandva-v4\",\"version\":\"0.4.2\",\"publish\":false,\"write_schema\":\"dvandva.run.v1\",\"read_schemas\":[\"dvandva.run.v1\"],\"role_api\":1,\"capabilities\":{\"upgrade_from_v1\":false},\"compatible\":true}'" \
  '  exit 0' \
  'fi' \
  'exit 2' \
...
if PATH="$fake_bin:$PATH" FAKE_KERNEL="$test_root/fake-kernel" \
  ...
  bash "$packager" skills-v0.4.2 "$incompatible" \
...
if PATH="$fake_bin:$PATH" FAKE_KERNEL="$test_root/wrong-version-kernel" \
  ...
  bash "$packager" skills-v0.4.2 "$wrong_binary" \
...
printf '%s\n' \
  ...
  '    valid|one_lf) printf "dvandva-v4 0.4.2\\n" ;;' \
  '    exact) printf "dvandva-v4 0.4.2" ;;' \
  '    nul) printf "dvandva-v4 0.4.2\\0\\n" ;;' \
  '    invalid_utf8) printf "dvandva-v4 0.4.2\\377\\n" ;;' \
  '    oversized) printf "dvandva-v4 0.4.2"; head -c 300 /dev/zero | tr "\\0" x ;;' \
  '    extra_newline) printf "dvandva-v4 0.4.2\\n\\n" ;;' \
  '    crlf) printf "dvandva-v4 0.4.2\\r\\n" ;;' \
  '    nonzero) printf "dvandva-v4 0.4.2\\n"; exit 7 ;;' \
  '    *) exit 2 ;;' \
  '  esac' \
  '  exit 0' \
  'fi' \
  ...
  "  valid) printf '%s\\n' '$valid_probe' ;;" \
  "  root_duplicate) printf '%s\\n' '{\"package\":\"wrong\",\"package\":\"dvandva-v4\",\"version\":\"0.4.2\",\"publish\":false,\"write_schema\":\"dvandva.run.v2\",\"read_schemas\":[\"dvandva.run.v2\",\"dvandva.run.v1\"],\"role_api\":2,\"capabilities\":{\"upgrade_from_v1\":true},\"compatible\":true}' ;;" \
  "  nested_duplicate) printf '%s\\n' '{\"package\":\"dvandva-v4\",\"version\":\"0.4.2\",\"publish\":false,\"write_schema\":\"dvandva.run.v2\",\"read_schemas\":[\"dvandva.run.v2\",\"dvandva.run.v1\"],\"role_api\":2,\"capabilities\":{\"upgrade_from_v1\":false,\"upgrade_from_v1\":true},\"compatible\":true}' ;;" \
...
expect_rejected_version() {
  ...
  if PATH="$fake_bin:$PATH" FAKE_KERNEL="$test_root/adversarial-kernel" \
    ...
    bash "$packager" skills-v0.4.2 "$rejected_output" \
...
expect_accepted_version() {
  ...
  if ! PATH="$fake_bin:$PATH" FAKE_KERNEL="$test_root/adversarial-kernel" \
    FAKE_VERSION_MODE="$mode" VERSION_DIR_FILE="$version_dir_file" \
    CARGO_TARGET_DIR="$test_root/$label-target" \
    bash "$packager" skills-v0.4.2 "$accepted_output" \
    >"$test_root/$label.out" 2>&1; then
...
if PATH="$fake_bin:$PATH" FAKE_KERNEL="$test_root/adversarial-kernel" \
  ...
  bash "$packager" skills-v0.4.2 "$empty_output" \
...
if PATH="$fake_bin:$PATH" FAKE_KERNEL="$test_root/adversarial-kernel" \
  ...
  bash "$packager" skills-v0.4.2 "$symlink_output" \
...
CARGO_TARGET_DIR="$test_root/target" bash "$packager" skills-v0.4.2 "$output"
...
test "$($output/dvandva-kernel-linux-x86_64 --version)" = 'dvandva-v4 0.4.2'
...
if PATH="$checksum_bin:$PATH" FAKE_KERNEL="$output/dvandva-kernel-linux-x86_64" \
  ...
  bash "$packager" skills-v0.4.2 "$checksum_output" \
...
if PATH="$collision_bin:$PATH" FAKE_KERNEL="$output/dvandva-kernel-linux-x86_64" \
  ...
  bash "$packager" skills-v0.4.2 "$collision_output" \
...
if CARGO_TARGET_DIR="$test_root/target" bash "$packager" skills-v0.4.2 "$nonempty" \
...
require_text 'skills-v0.4.2' "$workflow_doc"
...
require_text 'skills-v0.4.2' "$readme_active"
