...
Dvandva 1.2.0 ships the canonical Dvandva roster. Run 4 makes Dvandva-only
...
Run 4 retires only Dvandva-covered standalone agents after version 1.2.0 cache
...
Run 4 records the 1.2.0 Dvandva roster parity, Dvandva-only retirement, Codex
...
echo "Codex agent-axis no-op skills out of scope no skill touches backup manifest restore 1.2.0"
EOF
...
#!/usr/bin/env bash
echo "1.2.0 dvandva-adversarial-analyst dvandva-test-creator"
EOF

  cat > "$root/scripts/test-install.sh" <<'EOF'
#!/usr/bin/env bash
echo "1.2.0 canonical 15-agent roster"
EOF
...
expect_fail \
  ...
  "$CASE" \
  "Dvandva manifest versions must all equal 1.2.0"
