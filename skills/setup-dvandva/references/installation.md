...
The release target is `0.3.8` / `skills-v0.3.8`. The installer resolves the
tag and release asset at invocation and fails closed if either is missing.
The operations:

```bash
bash scripts/setup-dvandva.sh install --version 0.3.8
bash scripts/setup-dvandva.sh update --version 0.3.8
bash scripts/setup-dvandva.sh doctor --version 0.3.8
bash scripts/setup-dvandva.sh uninstall --version 0.3.8
```

These operations consume release `skills-v0.3.8`. Compatibility requires
